...
bool OnOwnerThread(const char* operation) {
    ...
    Log("%s called on tid=%d; sole owner is GameThread tid=%d", operation,
...
bool CreateSoleEglContext() {
    ...
    if (g.display == EGL_NO_DISPLAY) {
        Log("eglGetDisplay failed: 0x%x", eglGetError());
        return false;
    }
    EGLint eglMajor = 0;
    EGLint eglMinor = 0;
    if (eglInitialize(g.display, &eglMajor, &eglMinor) != EGL_TRUE) {
        Log("eglInitialize failed: 0x%x", eglGetError());
        return false;
    }
    if (eglBindAPI(EGL_OPENGL_ES_API) != EGL_TRUE) {
        Log("eglBindAPI(EGL_OPENGL_ES_API) failed: 0x%x", eglGetError());
        return false;
    }
    ...
    g.context = eglCreateContext(g.display, g.config, EGL_NO_CONTEXT,
                                 contextAttributes31);
    if (g.context == EGL_NO_CONTEXT) {
        Log("eglCreateContext(GLES 3.1) failed: 0x%x", eglGetError());
        return false;
    }
    ...
    if (major < 3 || (major == 3 && minor < 1)) {
        Log("librw requires GLES 3.1, but the created context is %d.%d",
            major, minor);
        return false;
    }
...
bool CreateSessionAndSpace() {
    ...
    Log("xrCreateSession succeeded on GameThread tid=%d",
...
bool Initialize() {
    ...
    Log("Ownership: GameActivity -> GameThread tid=%d -> sole EGLContext -> OpenXR",
        static_cast<int>(g.ownerThread));
...
void FinalizeAfterEngineShutdown() {
    if (g.display != EGL_NO_DISPLAY) {
        if (eglGetCurrentContext() == g.context) {
    ...
    Log("OpenXR and sole GameThread EGL ownership fully released");
    if (g.logFile != NULL) {
        fclose(g.logFile);
        g.logFile = NULL;
    }
...
extern "C" int RevcOpenXrRecommendedWidth(void) {
    return QuestOpenXR::RecommendedWidth();
}

extern "C" int RevcOpenXrRecommendedHeight(void) {
    return QuestOpenXR::RecommendedHeight();
}
